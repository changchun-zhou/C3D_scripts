...
import statistical_distribution_different_threshold
...
for net, shape_scale in act_dict.items():
    # if net == 'inputs' :
        ...
        print(' <<<< ', net,' >>>>')
        statistical_distribution_different_threshold.statistical_distribution(extract_dir=extract_src_dir, dequant_dir=dequant_dir, \
                name='Activation_'+str(file_epoch)+'_'+net,tensor=tensor,type='act',mode='dequant', scale=scale, theshold=threshold, \
                Number_Conv=Number_Conv, density_wei = density_wei[Number_Conv], MACs = MACs[Number_Conv], date_str= date_str, factor_sparsity=factor_sparsity[Number_Conv], file_name=args.name) # >= theshold
        Number_Conv += 1
plt.legend(fontsize=14)   #显示标签
plt.gca().yaxis.set_major_locator(MultipleLocator(5))
plt.ylim(0, 70 )
plt.xlim(0, 10)
plt.savefig(os.path.join(extract_src_dir)+'/statistical_distribution_bar_'+date_str+ '_' + args.name +'.svg', format='svg')
