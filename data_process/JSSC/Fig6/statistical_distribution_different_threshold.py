...
def  statistical_distribution(extract_dir, dequant_dir, name,tensor, type, mode,scale, theshold, Number_Conv, density_wei, MACs, date_str, factor_sparsity, file_name):
    ...
    for name_view, list_view in dict_list.items():
        ...
        print("Conv {} density after threshold".format(Number_Conv+1))
        print(100 - sparsity_th[0])
        for th in range(1,10):
            rate_cut_th.append( (100 - sparsity_th[th])/(100-sparsity_th[0])*100)
            print(100 - sparsity_th[th])
        # plt.bar(bar_width*2*(Number_Conv - 4) + np.linspace(1,9,9),rate_cut_th , \
        #     width=bar_width, color=(0.1*(Number_Conv+1), 0.1*(Number_Conv+1), 0.1*(Number_Conv+1)), label="Conv "+str(Number_Conv+1))
        position += 1
    ...
    # plt.legend(fontsize=16)   #显示标签
    # plt.gca().yaxis.set_major_locator(MultipleLocator(5))
    # plt.ylim(0, max(rate_cut_th) )
    # plt.xlim(-1, 11)
