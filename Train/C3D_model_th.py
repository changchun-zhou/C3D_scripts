...
class C3D(nn.Module):
    ...
    def threshold(self, tensor_in, scale, threshold = 1):
        ...
        select_indices = torch.where(torch.abs(diff) <= threshold)[0]
    ...
    def forward(self,x):
        ...
        if Switch :
            x = self.threshold(x, scale[0], 3)
        
        x = self.relu1(self.conv1(x))
        x = self.pool1(x)

        if Switch :
            x = self.threshold(x, scale[1], 2)

        x = self.relu2(self.conv2(x))
        x = self.pool2(x)

        if Switch :
            x = self.threshold(x, scale[2])
        print('finish threshold:'+str(timeit.default_timer()))
        torch.cuda.empty_cache()
        x = self.relu3(self.conv3a(x))
        # if Switch :
        #     x = self.threshold(x, scale[3])
