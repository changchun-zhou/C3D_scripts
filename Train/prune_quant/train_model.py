                    ...
                    with torch.no_grad():
                        ...
                        if (args.extract or args.dequant) and minibatch_id == 0 and epoch % save_epoch == 0:
                            ...
                            for [name,layer] in layers:
                                print("****** extract: **********", name)
                                # conv* or pool*
                                # if fnmatch(name,'*conv*') or fnmatch(name,'*pool*'):
                                if fnmatch(name,'fc*')==False and fnmatch(name,'dropout*')==False and fnmatch(name,'relu9*')==False and fnmatch(name,'relu10*')==False:
                                    internal_output = activation[name]
                                    Function_self.tensor_to_file(extract_dir=extract_dir,name='Activation_'+str(file_epoch)+'_'+name,tensor=internal_output,type='act', mode='extract', scale=1)
                            if args.dequant:
