...
def train_model(conf,model,optimizer,dataset, save_dir, saveName, num_classes, lr,
    ...
    for epoch in range(resume_epoch, nEpochs):
        ...
        for layer in range(3, 8):
            for sensitivity_threshold in range(80, 81, 10):
                print(" before modifying Threshold: {}".format(model.module.Threshold))
                for net, param in model.named_parameters():
                    if fnmatch(net, '*Threshold*'):
                        param.data = torch.Tensor([0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001, 0.001]).to(device)
                        param.data[layer] = torch.Tensor([sensitivity_threshold+0.001]).to(device)
                print(" after modifying Threshold: {}".format(model.module.Threshold))
                for phase in ['val']:
                    ...
                    for inputs, labels in tqdm(trainval_loaders[phase]):
                        ...
                        if phase == 'train':
                            ...
                            # model.Switch, model.scale = True, [1, 0.0646, 0.0351, 0.0329, 0.0303, 0.0392, 0.0551, 0.0870,0.1439, 1.6855, 0.3818]
                            #else:
                            ...
                            with torch.no_grad():
                                ...
                                #     model.Switch, model.scale = True, [1, 0.0646, 0.0351, 0.0329, 0.0303, 0.0392, 0.0551, 0.0870,0.1439, 1.6855, 0.3818]
                                # else:
                                #     model.Switch, model.scale = False, [1, 0.0646, 0.0351, 0.0329, 0.0303, 0.0392, 0.0551, 0.0870,0.1439, 1.6855, 0.3818]
                                torch.cuda.empty_cache()
                                outputs = model(inputs)
                                # if (conf.items('fine')['lambda'] ) and minibatch_id == 0 and epoch % save_epoch == 0:
                        ...
                        # min_loss = 0.048
                        # a = 0.1
                        # b = min_loss*torch.norm(torch.full([8], exp_th).to(device))*exp_th
                        for net,param in model.named_parameters():
                            if fnmatch(net, '*Threshold*'):
                                threshold = param
                        ...
                        if phase == 'train':
                            ...
                            # optimizer.zero_grad() 
                            if log_threshold_training:
                                #  msglogger.info('\n\r threshold grad: {} \n\r original threshold:  value: {}'.format(threshold.grad.cpu().detach().numpy(), threshold.cpu().detach().numpy()))
                ...
                #     # apputils.save_checkpoint(epoch+1,'my_model',model,optimizer=optimizer,scheduler=compression_scheduler,name='quantized_'+saveName+'_epoch-'+str(epoch),dir=os.path.join(save_dir,'models'))
                ...
                if useTest and epoch % test_interval == (test_interval - 1) and epoch >= conf.getint('set', 'epoch_start_valtest'):
                    ...
                    save_list_sensitivity = []
                    save_list_sensitivity.append( (epoch_acc_trainval+ epoch_acc)/2)
                    save_list_sensitivity.extend( (threshold.cpu().detach().numpy()).tolist() )
                    to_csv.to_csv(save_dir+'/sensitivity_valtest_acc.csv', epoch, save_list_sensitivity)
