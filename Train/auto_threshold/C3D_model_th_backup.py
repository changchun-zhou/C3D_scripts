...
class C3D(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False):
        ...
        self.Threshold = nn.Parameter(torch.Tensor(8), requires_grad = True)
        self.Threshold.data.fill_(1.) # 1/4
    ...
    def threshold(self, tensor_in, Threshold= 1): 
    ...
    def forward(self,x):
        ...
        x = self.threshold(x, self.Threshold[0])
        
        x = self.relu1(self.conv1(x))
        x = self.pool1(x)

        x = self.threshold(x,  self.Threshold[1])

        x = self.relu2(self.conv2(x))
        x = self.pool2(x)


        x = self.threshold(x, self.Threshold[2])
        # print('finish threshold:'+str(timeit.default_timer()))
        torch.cuda.empty_cache()
        x = self.relu3(self.conv3a(x))

        x = self.threshold(x,  self.Threshold[3])

        x = self.relu4(self.conv3b(x))
        x = self.pool3(x)


        x = self.threshold(x,  self.Threshold[4])

        x = self.relu5(self.conv4a(x))

 
        x = self.threshold(x, self.Threshold[5])
        x = self.relu6(self.conv4b(x))
        x = self.pool4(x)

        x = self.threshold(x,  self.Threshold[6])
        x = self.relu7(self.conv5a(x))


        x = self.threshold(x, self.Threshold[7])
