        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in tqdm(trainval_loaders[phase]):
                ...
                for net,param in model.named_parameters():
                    if fnmatch(net, '*Threshold*'):
                        threshold = param
                loss_th = b/(torch.norm(threshold) + a)
                loss = (loss_weight + loss_th)
                log_threshold_training = minibatch_id % (trainval_sizes[phase]/batch_size/100) ==0
                    ...
                    if log_threshold_training:
                        for net,param in model.named_parameters():
                            if fnmatch(net, '*Threshold*'):
                                ...
                                msglogger.info(' before grad, grad: {}, \n\r, param: {}'.format(param.grad, param))
                                # print('\n')
                    ...
                    if log_threshold_training:
                        for net,param in model.named_parameters():
                            if fnmatch(net, '*Threshold*'):
                                ...
                                # print('\n'*3)
                                msglogger.info(' after grad, grad: {}, \n\r, param: {}'.format(param.grad, param))
