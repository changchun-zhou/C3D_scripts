...
class C3D(nn.Module):
    ...
    def theshold(self, tensor_in, scale):
        ...
        for batch in range(batch_num):
            for channel in range(c_num):
                tensor_tmp = tensor_in[batch][channel]
                front, back = tensor_tmp[:-1], tensor_tmp[1:]
                diff = scale * (back - front)
                select_indices = torch.where(torch.abs(diff) <= 1)[0]
                diff[select_indices] = 0
                back = front + diff/scale
                tensor_out[batch][channel] = torch.cat([tensor_tmp[0].unsqueeze(0), back], dim=0)

                # tensor_tmp = tensor_in[batch][channel]
                # front, back = tensor_tmp[:-1], tensor_tmp[1:]
                # diff = scale * torch.abs(front - back)
                # select_indices = torch.where(diff <= 1)[0]
    ...
    def forward(self,x):
        ...
        if Switch :
            x = self.theshold(x, scale[0])

        x = self.relu1(self.conv1(x))
        x = self.pool1(x)

        if Switch :
            x = self.theshold(x, scale[1])

        x = self.relu2(self.conv2(x))
        x = self.pool2(x)

        if Switch :
            x = self.theshold(x, scale[2])

        x = self.relu3(self.conv3a(x))

        if Switch :
            x = self.theshold(x, scale[3])

        x = self.relu4(self.conv3b(x))
        x = self.pool3(x)

        if Switch :
            x = self.theshold(x, scale[4])

        x = self.relu5(self.conv4a(x))

        if Switch :
            x = self.theshold(x, scale[5])
