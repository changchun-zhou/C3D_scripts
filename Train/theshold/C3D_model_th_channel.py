...
class C3D(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False):
        ...
        self.Threshold_channel = nn.Parameter(torch.ones([8, 512]), requires_grad = True)
        self.Threshold_channel.data.fill_(0.5) # 1/4
        act_dict = {
                    [  3, 16, 112, 112],
                    [ 64, 16,  56,  56],
                    [128,  8,  28,  28],
        ...
        Threshold_conv1 = torch.zeros(act_dict[0])
        for i in range(8):
                    Threshold_conv1[act_dict[i][0]] = Threshold_channel[i][act_dict[i][0]]
        ...
        # self.Threshold.data.fill_(1.)
        # device = torch.device("cuda:1")
        # Threshold = Variable(10*torch.ones(8)).to(device)
    ...
    def threshold(self, tensor_in, scale, Threshold= 1): 
        ...
        Threshold = self.relu13(Threshold)

        Threshold = 10 + self.relu14(10 - Threshold)
...
def get_10x_lr_Threshold0(model):
    """
    This generator returns all the parameters for the last fc layer of the net.
    """
    yield model.Threshold0
