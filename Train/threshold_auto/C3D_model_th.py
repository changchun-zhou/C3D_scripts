...
class C3D(nn.Module):
    ...
    def __init__(self, num_classes, pretrained=False):
        ...
        self.Switch, self.scale = False, [1,1,1,1,1]
        self.Threshold = 50
    ...
    def threshold(self, tensor_in, scale, threshold = 1):
        ...
        # print('threshold size:',batch_num, c_num, f_num, h, w)
        ...
        #                        tensor_in[batch][channel][frame-1][height][width]) <= 1 : # Threshold
        ...
        # return  tensor_out
    def forward(self,x):
        # print('start inference:'+str(timeit.default_timer()))
        Layer_Depth = 2
        
        Switch, scale= self.Switch, self.scale

        if Layer_Depth >= 1 :
            x = self.threshold(x, scale[0], Threshold)
        
        x = self.relu1(self.conv1(x))
        x = self.pool1(x)

        if Layer_Depth >= 2 :
            x = self.threshold(x, scale[1], Threshold)

        x = self.relu2(self.conv2(x))
        x = self.pool2(x)

        if Layer_Depth >= 3 :
            x = self.threshold(x, scale[2], Threshold)
        # print('finish threshold:'+str(timeit.default_timer()))
        torch.cuda.empty_cache()
        x = self.relu3(self.conv3a(x))
        if Layer_Depth >= 4 :
            x = self.threshold(x, scale[3], Threshold)

        x = self.relu4(self.conv3b(x))
        x = self.pool3(x)

        if Layer_Depth >= 5 :
            x = self.threshold(x, scale[4], Threshold)

        x = self.relu5(self.conv4a(x))

        if Layer_Depth >= 6 :
            x = self.threshold(x, scale[5], Threshold)
        x = self.relu6(self.conv4b(x))
        x = self.pool4(x)

        if Layer_Depth >= 7 :
            x = self.threshold(x, scale[6], Threshold)
        x = self.relu7(self.conv5a(x))

        if Layer_Depth >= 8 :
            x = self.threshold(x, scale[7], Threshold)
